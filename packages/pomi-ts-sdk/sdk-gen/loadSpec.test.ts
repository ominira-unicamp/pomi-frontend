...
async function withSpec(document: unknown, run: (path: string) => Promise<void>) {
    ...
    paths: {
      '/examples': {
        get: {
          ...
          deprecated: true,
          'x-pomi-sdk': false,
          responses: { 200: { content: { 'application/json': { schema: { type: 'array', items: { $ref: '#/components/schemas/Example' } } } } } },
        },
      },
    },
...
test('rejects incomplete path mappings', async () => {
  await withSpec({
    ...
    paths: {
      '/examples/{id}': {
        get: {
          ...
          'x-pomi-sdk': { resource: 'examples', method: 'get', action: 'get' },
    ...
    await assert.rejects(loadSpec('app', path), /must map every path parameter/)
  })
})
