...
function operationEntries(target: ApiTarget, document: OpenApiDocument) {
  ...
  for (const [path, item] of Object.entries(document.paths ?? {})) {
    for (const [method, value] of Object.entries(item)) {
      ...
      entries.push({ target, path, method, operation: value as OpenApiOperation, document })
    }
  }
...
function schemaHasPath(document: OpenApiDocument, schema: unknown, path: string) {
...
function successfulSchemas(entry: OperationModel) {
  return Object.entries(entry.operation.responses ?? {})
    ...
    .flatMap(([, response]) => Object.values(response.content ?? {}).map((content) => content.schema))
}
...
function validateOperation(entry: OperationModel) {
  ...
  if (sdk === false) {
    if (entry.operation.deprecated !== true)
      throw new Error(`Only deprecated operation ${id} may opt out of SDK generation`)
    return
  }
  if (!sdk)
  ...
  if (paths.length !== mappings.length || paths.some((path) => !mappings.some(([wire]) => wire === path)))
    throw new Error(`Operation ${id} must map every path parameter`)
  const publicNames = new Set<string>()
  for (const [wireName, publicName] of mappings) {
    if (!paths.includes(wireName) || !/^[A-Za-z_$][A-Za-z0-9_$]*$/u.test(publicName) || publicNames.has(publicName))
  ...
  if (sdk.action !== 'list') throw new Error(`Pagination requires list action in operation ${id}`)
  if (pagination.defaultPageSize < 1 || (pagination.maxPageSize !== undefined && pagination.maxPageSize < pagination.defaultPageSize))
    throw new Error(`Invalid pagination limits in operation ${id}`)
  if (pagination.defaultMode === 'all' && !pagination.allowAll)
    throw new Error(`Pagination default all requires allowAll in operation ${id}`)
  ...
  for (const field of ['data', 'quantity', 'total', '_paths.next']) {
    if (!successfulSchemas(entry).some((schema) => schemaHasPath(entry.document, schema, field)))
      throw new Error(`Pagination response field ${field} does not exist in operation ${id}`)
  }
}
...
function validateSchemas(document: OpenApiDocument) {
  ...
  for (const [name, schema] of Object.entries(document.components?.schemas ?? {})) {
    ...
    if (!metadata?.kind || !metadata.publicName) throw new Error(`Missing x-pomi-schema in ${name}`)
    ...
    const fields = [...(metadata.transportFields ?? []), ...(metadata.identityFields ?? []), ...(metadata.readOnlyFields ?? []), ...Object.keys(metadata.relations ?? {})]
    for (const field of fields) if (!(field in properties)) throw new Error(`Schema ${name} metadata references missing field ${field}`)
  }
}

export async function loadSpec(target: ApiTarget, sourcePath: string) {
  const document = JSON.parse(await readFile(sourcePath, 'utf8')) as OpenApiDocument
  if (!document.openapi.startsWith('3.')) throw new Error(`Expected an OpenAPI 3 document: ${sourcePath}`)
  const entries = operationEntries(target, document)
  if (entries.length === 0) throw new Error(`No HTTP operations found in ${sourcePath}`)
  ...
  for (const entry of entries) {
    ...
    if (methods.has(key)) throw new Error(`Duplicate SDK operation in ${target}: ${key}`)
    methods.add(key)
  }
