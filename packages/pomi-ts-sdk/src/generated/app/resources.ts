...
async function* paginateByLink<Page extends { _paths: { next: string | null } }>(firstPage: Promise<Page>, target: "app", authentication: AuthenticationMode, requestPath: RequestPath, context?: PomiRequestContext): AsyncIterable<Page> {
...
export function bindResources(operations: Operations, requestPath: RequestPath) {
    ...
    return {
        ...
        "feedbackReports": (() => {
            ...
            const pages = (studentId: number, input: Omit<listStudentFeedbackReportsInput, "sid"> = {}, context?: PomiRequestContext) => paginateByLink<listStudentFeedbackReportsOutput>(operations.listStudentFeedbackReports(operationInput<listStudentFeedbackReportsInput>({ "sid": studentId, ...{ ...input, pageSize: input.pageSize ?? 'all' } }), context), "app", definitions.listStudentFeedbackReports.authentication, requestPath, context)
            const listAll = async (studentId: number, input: Omit<listStudentFeedbackReportsInput, "sid"> = {}, context?: PomiRequestContext) => { const items: Array<listStudentFeedbackReportsOutput['data'][number]> = []; for await (const page of pages(studentId, input, context)) items.push(...page.data); return items }
            return { create: createOperation, createForStudent: createForStudentOperation, listForStudent: listForStudentOperation, pages, listAll }
        })(),
        "studentAbsences": (() => {
